  ...
  public:
    ...
    void poll(int timeout) { // timeout in milliseconds
        ...
        while (true) {
            ...
            if (false) { }
                ...
                closesocket(sockfd);
                readyState = CLOSED;
                utlLog_error(ret < 0 ? "Connection error!" : "Connection closed!");
                break;
            }
            else {
                rxbuf.resize(N + ret);
    ...
    virtual void _dispatchBinary(BytesCallback_Imp & callable) {
        ...
        while (true) {
            ...
            else { utlLog_error("ERROR: Got unexpected WebSocket message."); close(); }
...
easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin) {
    ...
    char path[128];
    if (url.size() >= 128) {
      utlLog_error("ERROR: url size limit exceeded: %s", url.c_str());
      return NULL;
    }
    if (origin.size() >= 200) {
      utlLog_error("ERROR: origin size limit exceeded: %s", origin.c_str());
      return NULL;
    }
    if (false) { }
        ...
        utlLog_error("ERROR: Could not parse WebSocket url: %s", url.c_str());
        return NULL;
    }
    utlLog_debug("easywsclient: connecting: host=%s port=%d path=/%s", host, port, path);
    socket_t sockfd = hostname_connect(host, port);
    if (sockfd == INVALID_SOCKET) {
        utlLog_error("Unable to connect to %s:%d\n", host, port);
        return NULL;
    }
    {
        ...
        line[i] = 0;
        if (i == 255) { utlLog_error("ERROR: Got invalid status line connecting to: %s", url.c_str()); return NULL; }
...
#endif
    utlLog_debug("Connected to: %s", url.c_str());
