...
//#define SUPPORT_PRI_MAGIC 1  /* for kernel check */
/*
...
#ifdef SUPPORT_PRI_MAGIC
    {
        uint32_t ksz = ntohl(hdr->ih_ksz);
        uint32_t magic_pri = ntohl(hdr->ih_pmagic);

        if ( magic_pri != (checksum + ksz) )
        {
            printf("Image header info is wrong!");
        ...
        //printf ("checksum=0x%x magic_pri=0x%x ksz=0x%x\n", checksum, magic_pri, ksz);
    }
#endif
