...
int check_image_validation(void)
{
    ...
    /*
	printf("\n=================================================\n");
    printf("\nCFG_KERN_ADDR=0x%X,CFG_KERN2_ADDR=0x%X, CFG_FLASH_BASE=0x%X,CFG_BOOTLOADER_SIZE=0x%X,CFG_FACTORY_SIZE=0x%X\n",CFG_KERN_ADDR,CFG_KERN2_ADDR,CFG_FLASH_BASE,CFG_BOOTLOADER_SIZE,CFG_FACTORY_SIZE);
    printf("\nPHYS_FLASH_1=0x%x,PHYS_FLASH2_1=0x%X\n",PHYS_FLASH_1,PHYS_FLASH2_1);
    printf("\nCFG_KERN_SIZE=0x%X,CFG_CONFIG_SIZE=0x%X\n",CFG_KERN_SIZE,CFG_CONFIG_SIZE);
	printf("\n=================================================\n");
    printf("\n CFG_FLASH_BASE=0x%X!\n,CFG_FLASH_BASE");
	*/
	...
	if (broken1 == 0) {
		...
		if (crc32(0, (char *)CFG_SPINAND_LOAD_ADDR, len) != chksum1)
#else //CFG_ENV_IS_IN_FLASH
		if (crc32(0, (char *)(hdr1_addr + sizeof(image_header_t)), len) != chksum1)
#endif
		{
			broken1 = 1;
			printf("Failed\n");
	...
	if (broken2 == 0) {
		...
		chksum2 = ntohl(hdr2.ih_dcrc);
#if defined (CFG_ENV_IS_IN_NAND)
		ranand_read((char *)CFG_SPINAND_LOAD_ADDR,
				(unsigned int)hdr2_addr - CFG_FLASH_BASE + sizeof(image_header_t),
				len);
		if (crc32(0, (char *)CFG_SPINAND_LOAD_ADDR, len) != chksum2)
#elif defined (CFG_ENV_IS_IN_SPI)
		raspi_read((char *)CFG_SPINAND_LOAD_ADDR,
				(unsigned int)hdr2_addr - CFG_FLASH_BASE + sizeof(image_header_t),
				len);
		if (crc32(0, (char *)CFG_SPINAND_LOAD_ADDR, len) != chksum2)
#else //CFG_ENV_IS_IN_FLASH
		if (crc32(0, (char *)(hdr2_addr + sizeof(image_header_t)), len) != chksum2)
#endif
		{
			broken2 = 1;
			printf("Failed\n");
	...
	else
    {
        if (chksum1 != chksum2)
        {
            len = ntohl(hdr1.ih_size) + sizeof(image_header_t);
            if (len > CFG_KERN2_SIZE)
                printf("\nImage2 is need update, but Image1 size(0x%X) is too big(limit=0x%X)!!\
                        \nGive up copying image.\n", len, CFG_KERN2_SIZE);
            else {
                printf("\nImage2 is need update. Copy Image1 to Image2.\n");
                copy_image(1, len);
            }
        }
		ret = -1;
 ...
 ************************************************************************
 */
int ub_check(char *load_address,int ubootlen)
{
	int crc_number = 0;
	int ret = 0;
	//int i;
	...
	if(load_address+ubootlen-4)
	{
		memcpy((char *)&crc_number,(char *)(load_address+ubootlen-4),4);	
		printf("crc_number = %x\n",crc_number);
		p = (unsigned char*)&crc_number;
	}

	ubootlen -= 4;
	if(crc_number == ((ubootlen*2)&0x77777777)){
...
void swap32(unsigned int * pulValue)
 {
	 if (NULL == pulValue)
	 {
		 return;
...
#define IMAGE_LINE  0x50040
int kernel_check(char *load_address,int kernellen,enum image_type type)
{
	int crc_number = 0;
	int end_check = 0;
	int ret = 0;
	...
	if(type == ONLY_KERNEL){
		if(load_address+60)
		{
			memcpy((char *)&crc_number,(char *)(load_address+60),4);	
			
		}
	}
	else if(type == FULL_IMAGE)
	{
		if(load_address+IMAGE_LINE-4)
		{
			memcpy((char *)&crc_number,(char *)(load_address+IMAGE_LINE-4),4);	
		}
	}
	else
	{
	...
	swap32(&crc_number);
	if(load_address+kernellen-4)
	{
		memcpy((char *)&end_check,(char *)(load_address+kernellen-4),4);	
	}
	
	if(end_check == ((crc_number*2)&0x77777777)){
		printf("check success.\n");
		ret = 1;
	}
	else
...
void board_init_r (gd_t *id, ulong dest_addr)
{
	...
	/*start of make rt5350 wireless led off by zengxianzhu at 20110607*/
        #if defined (RT5350_ASIC_BOARD)
        u32 regvalue;
	 ...
	 printf("******************************\n");
        ...
        *(volatile u_long *)(REG_3052_GIO_ADDRESS_LED) = regvalue;
        #endif
        /*end of make rt5350 wireless led off by zengxianzhu at 20110607*/
	...
	if(BootType == '3') {
		...
		case '9':
			...
			if (0) {//modify by zengxianzhu
				...
				unsigned int load_address = simple_strtoul(argv[1], NULL, 16);
				if(NetBootFileXferSize > 400000){
					Imgtype = FULL_IMAGE;
                    printf("Upgrade the whole image !\n");
					/*check_ret = kernel_check(load_address,NetBootFileXferSize,Imgtype); //check image*/
				}
				else
                {
                    printf("Upgrade uboot !\n");
                    /*check_ret = ub_check((char *)load_address,NetBootFileXferSize); //check uboot header*/
                }
				if(check_ret != 1)
					break;
