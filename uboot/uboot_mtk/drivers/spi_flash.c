...
static struct chip_info chips_data [] = {
	...
	{ "GD25Q256C",	    0xc8, 0x4019c840, 64 * 1024, 512, 1 },
...
int raspi_4byte_mode(int enable)
{
    ...
    else if (spi_chip_info->id == 0x20)  /* Micron, actually */
    {
        ...
        raspi_wait_ready(1);
        raspi_write_enable();  /* Some Micron need WREN command; all will accept it */
...
int raspi_erase_write(char *buf, unsigned int offs, int count)
{
	...
	ra_dbg("%s: offs:%x, count:%x\n", __func__, offs, count);
       sprintf(strCount,"%x", count);
       /*start of modify for upgrade flash image by zengxianzhu at 20120518*/
       if (strstr(tftp_filename, ".img") && strcmp(tftp_filename,"uboot.img")!=0)
       {
                //do nothing;
       }
       else
       ...
       /*end of modify for upgrade flash image by zengxianzhu at 20120518*/
...
int do_flerase (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	/*start of modify for  add erase nvram  by wangjian at 20111130*/
	else if (strcmp(argv[1], "nvram") == 0) {
		raspi_unprotect();
		rcode = raspi_erase(size - 0x0010000, 0x0010000);   /*last 64kb is nvram zone*/
		return rcode;		
	}
	/*end of modify for  add erase nvram by wangjian at 20111130*/
...
U_BOOT_CMD(
	...
	"erase uboot\n    - erase uboot block\n"
	"erase linux\n    - erase linux kernel block\n"
	"erase nvram\n    - erase nvram block\n"        /*modify for  add erase nvram  by wangjian at 20111130*/
);
